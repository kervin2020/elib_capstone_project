...
const AdminPage = () => {
  ...
  const handleBanUser = (userId, isBanned) => {
    setConfirmAction({
      ...
      userId,
      isBanned,
      message: `Are you sure you want to ${isBanned ? 'unban' : 'ban'} this user?`
    });
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      switch (confirmAction.type) {
        case 'ban':
          result = await toggleUserBan(confirmAction.userId, confirmAction.isBanned);
          if (result.success) {
            success(`User ${confirmAction.isBanned ? 'unbanned' : 'banned'} successfully`);
          }
          break;
        case 'deleteUser':
          result = await deleteUser(confirmAction.userId);
          if (result.success) {
            success('User deleted successfully');
          }
          break;
        case 'deleteBook':
          result = await deleteBook(confirmAction.bookId);
          if (result.success) {
            success('Book deleted successfully');
          }
          break;
        case 'deleteCategory':
          result = await deleteCategory(confirmAction.categoryId);
          if (result.success) {
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                      {loan.user?.username} borrowed "{loan.ebook?.title}"
                    </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage your library system, users, and content
          </p>
        </div>
