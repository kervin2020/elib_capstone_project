...
describe('User Flow Integration Tests', () => {
  ...
  test('user can borrow a book when authenticated', async () => {
    ...
    mockedAxios.get.mockResolvedValueOnce({
      data: { id: 1, username: 'testuser', is_admin: false }
    });
    ...
    mockedAxios.get
      .mockResolvedValueOnce({ data: { ebooks: mockBooks } })
      .mockResolvedValueOnce({ data: { loans: [] } });

    mockedAxios.post.mockResolvedValueOnce({
      data: { loan: { id: 1, ebook_id: 1, user_id: 1 } }
    });
    ...
    await waitFor(() => {
      expect(screen.getByText('Borrowable Book')).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(mockedAxios.post).toHaveBeenCalledWith('/api/loans', {
        ebook_id: 1
      });
    });
  });
  ...
  test('non-admin user cannot access admin dashboard', async () => {
    ...
    localStorageMock.getItem.mockReturnValue('mock-token');
    mockedAxios.get.mockResolvedValueOnce({
      data: { id: 1, username: 'user', is_admin: false }
    });
  ...
  test('user can return a book', async () => {
    ...
    mockedAxios.put.mockResolvedValueOnce({
      data: { loan: { id: 1, is_returned: true } }
    });
    ...
    await waitFor(() => {
      expect(mockedAxios.put).toHaveBeenCalledWith('/api/loans/1');
    });
  });
});
