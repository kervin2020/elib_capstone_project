...
export const BookProvider = ({ children }) => {
  ...
  const fetchBooks = async () => {
    ...
    try {
      ...
      dispatch({
        type: BOOK_ACTIONS.FETCH_BOOKS_SUCCESS,
        payload: response.data.ebooks
      });
    } catch (error) {
      dispatch({
        type: BOOK_ACTIONS.SET_ERROR,
        payload: error.response?.data?.msg || 'Erreur lors du chargement des livres'
      });
    }
  };
  ...
  const fetchCategories = async () => {
    try {
      ...
      dispatch({
        type: BOOK_ACTIONS.FETCH_CATEGORIES_SUCCESS,
  ...
  const fetchBook = async (bookId) => {
    ...
    try {
      const response = await axios.get(`/ebooks/${bookId}`);
      dispatch({
        type: BOOK_ACTIONS.FETCH_BOOK_SUCCESS,
        payload: response.data
      });
    } catch (error) {
      dispatch({
        type: BOOK_ACTIONS.SET_ERROR,
        payload: error.response?.data?.msg || 'Erreur lors du chargement du livre'
      });
    }
  };
  ...
  const addBook = async (bookData) => {
    ...
    try {
      ...
      dispatch({
        type: BOOK_ACTIONS.ADD_BOOK_SUCCESS,
        payload: response.data.ebook
      });
      return { success: true };
    } catch (error) {
  ...
  const updateBook = async (bookId, bookData) => {
    ...
    try {
      const response = await axios.put(`/ebooks/${bookId}`, bookData);
      dispatch({
        type: BOOK_ACTIONS.UPDATE_BOOK_SUCCESS,
        payload: response.data.ebook
      });
      return { success: true };
    } catch (error) {
  ...
  const deleteBook = async (bookId) => {
    ...
    try {
      ...
      dispatch({
        type: BOOK_ACTIONS.DELETE_BOOK_SUCCESS,
        payload: bookId
      });
      return { success: true };
    } catch (error) {
  ...
  const filterByCategory = (categoryId) => {
    dispatch({ type: BOOK_ACTIONS.SET_SELECTED_CATEGORY, payload: categoryId });
  };
  ...
  const getFilteredBooks = () => {
    ...
    if (state.selectedCategory) {
      filteredBooks = filteredBooks.filter(book =>
        book.categories && book.categories.some(cat => cat.id === state.selectedCategory)
      );
    }
