...
const LoansPage = () => {
  const { user } = useAuth();
  const { loans, stats, isLoading, fetchLoans, returnBook, getUserActiveLoans, getUserLoanHistory, getOverdueLoans } = useLoans();
  ...
  const confirmReturn = async () => {
    ...
    try {
      ...
      if (result.success) {
        success(`Successfully returned "${returningLoan.ebook?.title}"`);
        fetchLoans();
      } else {
  ...
  const getStatusColor = (loan) => {
    ...
    if (new Date(loan.due_date) < new Date()) return 'text-red-600 bg-red-100';
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">{loan.ebook?.title}</h3>
                <p className="text-gray-600">{loan.ebook?.author}</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage your borrowed books and view your loan history
          </p>
        </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {loanHistory.filter(loan => loan.is_returned).length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {activeLoans.filter(loan => new Date(loan.due_date) < new Date()).length}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{loanHistory.length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <ConfirmModal
          ...
          title="Return Book"
          message={`Are you sure you want to return "${returningLoan?.ebook?.title}"?`}
