...
// Types d'actions 
const TOAST_ACTIONS = {
 ADD_TOAST: 'ADD_TOAST', REMOVE_TOAST: 'REMOVE_TOAST', CLEAR_ALL: 'CLEAR_ALL' 
}
;
...
const toastReducer = (state, action)
 ...
 switch (action.type)
 {
 case TOAST_ACTIONS.ADD_TOAST: return {
 ...state, toasts: [...state.toasts, action.payload] 
}
;
 case TOAST_ACTIONS.REMOVE_TOAST: return {
 ...state, toasts: state.toasts.filter(toast => toast.id !== action.payload)
 
}
;
 case TOAST_ACTIONS.CLEAR_ALL: return {
 ...state, toasts: [] 
}
;
...
const newToast = {
 id, type: 'info', duration: 5000, ...toast 
}
;
 dispatch({
 type: TOAST_ACTIONS.ADD_TOAST, payload: newToast 
}
)
;
 return id;
...
const removeToast = (id)
 => {
 dispatch({
 type: TOAST_ACTIONS.REMOVE_TOAST, payload: id 
}
)
;
...
const error = (message, title = 'Error', options = {
 ...
 => {
 return addToast({
 type: 'error', title, message, duration: 7000, 
// Plus long pour les erreurs ...options 
}
)
;
...
const value = {
 ...state, addToast, removeToast, clearAll, success, error, warning, info 
}
;
