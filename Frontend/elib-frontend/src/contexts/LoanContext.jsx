...
export const LoanProvider = ({ children }) => {
  ...
  const fetchLoans = async () => {
    ...
    try {
      ...
      dispatch({
        type: LOAN_ACTIONS.FETCH_LOANS_SUCCESS,
  ...
  const fetchLoan = async (loanId) => {
    ...
    try {
      ...
      dispatch({
        type: LOAN_ACTIONS.FETCH_LOAN_SUCCESS,
  ...
  const createLoan = async (bookId) => {
    ...
    try {
      const response = await axios.post('/loans', { ebook_id: bookId });
      dispatch({
        type: LOAN_ACTIONS.CREATE_LOAN_SUCCESS,
  ...
  const returnBook = async (loanId) => {
    ...
    try {
      ...
      dispatch({
        type: LOAN_ACTIONS.UPDATE_LOAN_SUCCESS,
        payload: response.data.loan
      });
      return { success: true };
    } catch (error) {
  ...
  const deleteLoan = async (loanId) => {
    ...
    try {
      ...
      dispatch({
        type: LOAN_ACTIONS.DELETE_LOAN_SUCCESS,
        payload: loanId
      });
      return { success: true };
    } catch (error) {
  ...
  const calculateStats = (loans) => {
    ...
    const stats = {
      ...
      activeLoans: loans.filter(loan => !loan.is_returned).length,
      overdueLoans: loans.filter(loan => 
        !loan.is_returned && new Date(loan.due_date) < now
      ).length,
      returnedLoans: loans.filter(loan => loan.is_returned).length
    };
  ...
  const getUserActiveLoans = (userId) => {
    return state.loans.filter(loan => 
      loan.user_id === userId && !loan.is_returned
    );
  };
