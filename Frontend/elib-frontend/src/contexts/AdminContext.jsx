...
export const AdminProvider = ({ children }) => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: error.response?.data?.msg || 'Erreur lors du chargement des utilisateurs'
      });
    }
  };
  ...
  const fetchCategories = async () => {
    try {
      ...
      dispatch({
        type: ADMIN_ACTIONS.SET_ERROR,
        payload: error.response?.data?.msg || 'Erreur lors du chargement des catégories'
      });
    }
  };
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      ...
      dispatch({
        type: ADMIN_ACTIONS.DELETE_USER_SUCCESS,
        payload: userId
      });
      return { success: true };
    } catch (error) {
  ...
  const toggleUserBan = async (userId, isBanned) => {
    return await updateUser(userId, { is_banned: isBanned });
  };
